...
export function IBANValidation(): ValidatorFn {
  return (control: AbstractControl): { [key: string]: any } | null => {
    ...
    if (/[a-zA-Z]{2}[0-9]{20}$/g.test(control.value)) {
      const letterValue = control.value.substr(control.value.length - 1);
      const numberValue = control.value.substr(0, control.value.length - 1);
...
@Directive({
  selector: '[greater_than][formControlName],[greater_than][formControl],[greater_than][ngModel],[element],[scope],[attrs]',
...
export class greaterThanValidator implements Validator {
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    ...
    if(parseInt(control.value) <= 100 && parseInt(control.value) >= 5){

    } else {
      throw new Error('salta?');
    }
    return null;
...
export const MIS_VALIDADORES = [UppercaseValidator, NIFValidator, TypeValidator, EqualValidator,IBANValidator,greaterThanValidator, ]
